...
async function listVertexAIModels() {
    try {
        ...
        console.log('\n=== Testing with current config model: gemini-2.5-flash-lite ===\n');
        
        try {
            const currentModel = vertexAI.getGenerativeModel({
            ...
            console.log('✅ gemini-2.5-flash-lite - Image analysis WORKS');
            console.log('Response preview:', result.response.candidates[0].content.parts[0].text.substring(0, 200) + '...');
            
        } catch (error) {
            console.log('❌ gemini-2.5-flash-lite - Image analysis FAILED');
            console.log('Error details:', error.message);
        }
